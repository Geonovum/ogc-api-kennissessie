    ...
    if (_query.datetime) {
      ...
      if (datetimes.length == 1) {
        features = features.filter(
          element => dates.compare(element.properties[datetimeAttribName].toString(), datetimes[0]) == 0)
      }
      else {
        var beginDate = datetimes[0]
        var endDate = datetimes[1]
        if (beginDate != '..' && endDate != '..') { // bounded
          features = features.filter(
            element => dates.inRange(element.properties[datetimeAttribName].toString(), datetimes[0], datetimes[1]))
        }
        else if (beginDate == '..' && endDate != '..') { // half-bounded [.., date]
          features = features.filter(
            element => dates.until(element.properties[datetimeAttribName].toString(), datetimes[1]))
        }
        else if (beginDate != '..' && endDate == '..') { // half-bounded [date, ..]
          features = features.filter(
