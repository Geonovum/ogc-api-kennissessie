...
import sortables from "../../../models/common/collections/sortables.js";
...
export function get(req, res) {
  ...
  sortables.get(formatFreeUrl, format, collectionId, function (err, content) {
    if (err) {
      res
        .status(err.httpCode)
    ...
    switch (format) {
      ...
      case `html`:
        ...
        // This recommendation does not apply, if there are a large number of links included
        // in a response or a link is not known when the HTTP headers of the response are created.
        res.status(200).render(`sortables`, { content, serviceUrl });
        break;
      default:
        res.status(400).json({
